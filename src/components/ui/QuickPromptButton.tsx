...
export const QuickPromptButton: React.FC<QuickPromptButtonProps> = ({ item, onClick }) => {
	...
	return (
		<button
			...
			className="group flex w-full items-center gap-3 rounded-xl border border-slate-700 bg-slate-800/80 p-3 text-left transition-all hover:border-slate-600 hover:bg-slate-700/80 cursor-pointer "
		>
			...
			<span className="shrink-0 text-slate-500 transition-transform group-hover:translate-x-0.5"><ArrowRight className="h-5 w-5" /></span>
		</button>
	);
};
